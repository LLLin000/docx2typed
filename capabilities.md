...
| `foreign_edit_adopt(candidate, candidate_id?, consent_token?, family_id?, base_version?, target?, provenance?)` | Re-extract, attribute and gate an external candidate, then adopt it as the next Version (`baseline-transition`). Admission reads the store receipt, so a forged file beside the candidate cannot widen lineage. No receipt → manual entry requires an explicit `family_id` + `base_version`; similarity never picks either. A narrow target whose paragraph identity or insertion position rests on order pairing alone refuses (`foreign-identity-unproven`). The `word.foreign-edit.media-add` owner admits exactly one new PNG drawing inside the selected paragraph plus its new image relationship and `image/png` content-type default; existing media, relationship retargets, relationship-id reuse, lost parts, other package changes, and out-of-target drawings refuse. All other opaque/package loss, out-of-target semantic change, HEAD drift, and ambiguous identity refuse and are not overridable by consent. `provenance` optionally records what edited the candidate as caller-declared evidence (`tool`/`version`/`binary_sha256?`/`argv`/`exit_code`, argv redacted); no gate reads it, and an unknown field or an `engine-observed` claim refuses (`foreign-provenance-invalid`). |
